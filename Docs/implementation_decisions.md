...
- **`ConvertedTotal`/`ConvertedCurrency` snapshotting — confirmed** per §17's original recommended default (full detail there, not repeated here).
- **AI-assist provider/model/key configuration** — originally resolved as environment-variables-only, no database table, with the in-app "Configure" entry point as a read-only env-derived summary. **Superseded by M16** (`/grill-me` session, 2026-07-17): this is reversed — config now lives in a DB-backed `/settings` page (encrypted API keys, ordered free-text model lists, a cross-provider cascade order), with the old env vars removed entirely except a new `SETTINGS_ENCRYPTION_KEY`. Full current shape and motivation in §19 above; full build plan in `Docs/execution_plan.md` M16.
...
- **Eventual production hosting target — Vercel + Neon** (full detail in §2/§21), captured in `Docs/execution_plan.md` §15. Does not change §21: MVP build itself still focuses exclusively on local development.
