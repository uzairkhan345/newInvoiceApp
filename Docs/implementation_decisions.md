...
- **AI-assist provider/model/key configuration — resolved as environment variables only**, not a database table. `AI_PRIMARY_PROVIDER`, `AI_PRIMARY_MODEL`, `AI_FALLBACK_SEQUENCE`, and per-provider API key env vars are the only configuration surface. The in-app "Configure" entry point (§19, Story 11.4) is a **read-only** display of the current env-derived configuration — changing provider/model requires editing env vars and redeploying/restarting, not an in-app mutation. This still satisfies the rule that all three AI-assist forms work perfectly with the panel absent/misconfigured. **Superseded, M16 (`/grill-me` session, 2026-07-17):** this env-vars-only decision is reversed. Motivation: on both realistic deployment targets (Vercel, a DigitalOcean droplet — §21), an env-var change does *not* take effect without a redeploy/process-restart, which defeats "configure once, used from that point onward." The DB is already a hard dependency on every target being considered, so moving config there adds no new infrastructure. New shape: a Prisma model (one row per fixed provider — `google`/`anthropic`/`groq`, no dynamic provider list — holding an **encrypted** API key, an ordered free-text model list, an `enabled` flag, and a cascade `order`); a new `/settings` page (also linked from the AI-assist panel's "Configure" entry point, replacing the old read-only summary) with exactly 3 fixed provider cards, no add/remove; keys are write-only from the UI (masked once set, never re-decrypted to the client, no test-call-on-save validation). Runtime fallback now walks two levels — providers in `order`, then each provider's models in list order — with the exact same catch-all/log-and-continue failure handling as before, just restructured from a flat sequence to a nested one. A new `SETTINGS_ENCRYPTION_KEY` env var is added (the only AI-related env var remaining) to encrypt keys at rest. Explicitly **not** solved by this milestone: the settings page has no auth, consistent with the rest of the app's current no-auth posture — flagged as a hard blocker before deploying anywhere reachable, not this milestone's job to fix. See `Docs/execution_plan.md` M16 for the full build plan.
