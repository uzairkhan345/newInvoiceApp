...
- **Below the fold**: a `2fr/1fr` row — **Project billing status** (the dark-header grid table, §9, scoped to active projects: Project / Last invoice / Period covered / Next invoice / Exposure / Status — no separate Billing/cadence column, redundant with "Period covered"/"Next invoice" and dropped to free up column width; "Period covered" is the last SENT/PAID invoice's `PeriodStart`–`PeriodEnd`, "—" when unset) alongside two stacked cards, **Upcoming billing** (next 14 days) and **Receivables ageing** (a single stacked bar bucketed Not yet due / 1–30 days / 31+ days, USD-only per §17). The Status column shows the status pill plus up to 3 small muted detail lines below it (invoices overdue/sent/in draft, each only when its count is nonzero) — never all 3 at once for a single project in practice, since overdue/sent are mutually exclusive subsets of the same SENT invoices. On the Health view's unfiltered "All projects" chip only, each row also gets up/down reorder arrows in a narrow leading column — they move the project within the global manual order that also drives the Projects page's card/table order (`Project.sortOrder`, one global order, no per-user concept). The arrows are hidden on every filtered chip (Needs attention/Upcoming/Healthy/the two stat-card filters) since a filtered subset's visible neighbor isn't reliably the same project the reorder would actually swap with.
