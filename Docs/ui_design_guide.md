...
- **Below the fold**: a `2fr/1fr` row — **Project billing status** (the dark-header grid table, §9, scoped to active projects: Project / Billing / Last invoice / Period covered / Next invoice / Exposure / Status — "Period covered" is the last SENT/PAID invoice's `PeriodStart`–`PeriodEnd`, "—" when unset) alongside two stacked cards, **Upcoming billing** (next 14 days) and **Receivables ageing** (a single stacked bar bucketed Not yet due / 1–30 days / 31+ days, USD-only per §17).
