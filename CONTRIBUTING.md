...
- **Read the decision record first** — [`Docs/implementation_decisions.md`](Docs/implementation_decisions.md) explains _why_ things are the way they are (no tax fields, snapshot locking at `SENT`, the currency model, deletion rules). PRs that contradict a documented decision should argue with the decision in the PR description, not silently reverse it.
